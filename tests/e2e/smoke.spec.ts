...
test.describe('Smoke Tests - Surfaces stables', () => {
  ...
  test('Settings charge et ouvre une sous-page stable', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Impôts', exact: true })).toHaveCount(0);
    await expect(
      page.getByRole('button', { name: 'Comptables & sociétés', exact: true }),
    ).toHaveCount(0);
    await expect(page.getByRole('button', { name: 'Paramètres sociaux', exact: true })).toHaveCount(
      0,
    );
    await expect(
