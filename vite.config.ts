...
export default defineConfig(({ mode }) => {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    console.warn(
      '[vite.config] ⚠️ VITE_SUPABASE_URL ou VITE_SUPABASE_ANON_KEY manquant dans .env.local',
    );
    console.warn(
      '[vite.config] Le proxy /api/admin sera désactivé. Voir .env.example pour la configuration.',
    );
  }
