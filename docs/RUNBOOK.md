...
- Pour changer le mot de passe d'un compte E2E sans email de récupération, utiliser l'API Admin
  ...
  navigateur :
  ```powershell
  $env:SUPABASE_SERVICE_ROLE_KEY = "<service-role-ou-sb-secret-key>"; $env:NEW_E2E_PASSWORD = "<nouveau-mot-de-passe>"; node -e "const { createClient } = require('@supabase/supabase-js'); const url = process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL; const key = process.env.SUPABASE_SERVICE_ROLE_KEY; const password = process.env.NEW_E2E_PASSWORD; if (!url || !key || !password) throw new Error('SUPABASE_URL/VITE_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY et NEW_E2E_PASSWORD requis'); const supabase = createClient(url, key, { auth: { persistSession: false, autoRefreshToken: false } }); supabase.auth.admin.updateUserById('b382d252-9de6-4721-9462-968ba48b7911', { password }).then(({ error }) => { if (error) throw error; console.log('Mot de passe E2E mis à jour'); }).catch((error) => { console.error(error.message); process.exit(1); });"
  ```
