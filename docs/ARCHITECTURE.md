...
| `/audit`                  | privé + lazy | `AuditPage`             | `src/features/audit/AuditPage.tsx` (exporté via `src/features/audit/index.ts`, réservé à la route lazy) — API partagée hors route dans `src/features/audit/shared.ts` pour Home/Strategy, expérience cockpit unique sans wizard horizontal, avec draft de session et sauvegarde locale JSON. Aucun export PPTX audit runtime. |
| `/strategy`               | privé + lazy | `StrategyPage`          | `src/pages/StrategyPage.tsx` (lazy) — workflow actif dépendant d'un draft d'audit, avec `SimFieldShell` pour la saisie produit. Aucun export PPTX Strategy runtime n'est exposé.                                                                                                                                              |
...
- Frontière exports : `src/pptx/**` ne doit jamais importer `src/features/**` ; les types et helpers partagés vivent dans `src/domain/**`, `src/engine/**`, `src/reporting/**` ou dans un module PPTX dédié. `/audit` et `/strategy` n'exposent plus de wrapper PPTX runtime.
...
- `src/features/<slug>/index.ts` : API publique de route de la feature, importable depuis `src/routes/` ou une autre feature
- `src/features/<slug>/shared.ts` : API publique optionnelle pour les helpers/types partagés quand `index.ts` doit rester un entrypoint lazy strict
