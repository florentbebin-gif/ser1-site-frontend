...
- Frontière exports : `src/pptx/**` ne doit jamais importer `src/features/**` ; les types et helpers partagés vivent dans `src/domain/**`, `src/engine/**`, `src/reporting/**` ou dans un module PPTX dédié. `/audit` n'a plus de wrapper PPTX runtime ; `src/features/strategy/export/exportStrategy.ts` reste le wrapper feature-owned de `src/pptx/strategyPptx.ts`.
