...
| `src/reporting/snapshot/snapshotSimRegistry.ts:26`, `src/reporting/snapshot/snapshotSimRegistry.ts:123`, `src/reporting/snapshot/snapshotSimRegistry.ts:139`, `src/reporting/snapshot/snapshotIO.ts:48`, `src/reporting/snapshot/snapshotIO.ts:76` | Les sauvegardes actuelles reposent sur des contrats explicites `sessionStorage` collectés/restaurés dans le format `.ser1`.                                                      | Cela ne suffit pas pour l'historique dossier, les versions, l'activation de stratégie et la reprise d'analyse patrimoniale. | Introduire un modèle de version de dossier avant d'étendre les parcours.                                                       |
...
Note F4d : le sous-pack livré sur charges sociales dirigeant est `partial` tant que seul le seuil
dividendes TNS est sourcé, éditable et consommé par `tresorerie-societe`. Rémunération TNS,
rémunération assimilé salarié, TA/TB/TC et Madelin restent bloqués avant passage `ready`.
...
| UX-03a.0 ✅ | Suppression export PPTX audit legacy                                                      | UX-01           | ✅ livré            | Aucun bouton/menu PPTX audit ; wrappers/builders/tests/docs audit PPTX supprimés ; sauvegarde locale JSON conservée                                                                                                 |
| UX-03a.1 ✅ | Foyer & famille                                                                           | F1 ✅, UX-00b   | ✅ livré            | Page cockpit en 4 cartes F1 + `AuditDrawer` minimal ; rail, topbar et barre d'état conservés ; aucune donnée famille locale concurrente                                                                             |
| UX-03a.2 ✅ | Inventaire actifs/passifs déclaratif                                                      | F1/DossierAudit | ✅ livré            | Page cockpit minimaliste : inventaire saisi, données partielles, à structurer F3 ; pas de patrimoine net, droits PP/US/NP ni droits successoraux                                                                    |
| UX-03a.3 ✅ | Fiscalité déclarative                                                                     | F1/DossierAudit | ✅ livré            | Page cockpit fiscalité déclarative initiale ; l'estimation IR/TMI est désormais portée par UX-06a partiel via moteur existant ; aucun hardcode fiscal, aucune optimisation réelle                                   |
| UX-03a.4 ✅ | Objectifs & suppression finale wizard                                                     | F1 ✅           | ✅ livré            | Page cockpit Objectifs séparée ; suppression visuelle du wizard horizontal, des étapes et des boutons précédent/suivant legacy                                                                                      |
| UX-03a.5 ✅ | Polish visuel pages internes `/audit`                                                     | UX-03a.4 ✅     | ✅ livré            | Cartes compactes, notes de périmètre et drawers premium ; visuels déclaratifs sans ratio trompeur ; aucun calcul métier nouveau                                                                                     |
| UX-03a.6 ✅ | Redesign pivot `Foyer & famille`                                                          | UX-03a.5 ✅     | ✅ livré            | Synthèse foyer typographique, filiation sobre, avatars sélectionnables, cartes de saisie compactes ; aucune bande Points prioritaires sur pages internes                                                            |
| UX-03b      | Page Budget & capacité                                                                    | F1.1            | après F1.1          | Synthèse budget éditable ; pas de détail mensuel                                                                                                                                                                    |
| UX-05       | Patrimoine réel F3 / actifs / passifs (tableau 5 colonnes PP/US/NP)                       | F3 ❌           | ⛔ gated F3         | Remplacera l'inventaire déclaratif UX-03a.2 par le graphe F3 ; patrimoine net = dossier central ; pas de recalcul local                                                                                             |
| UX-06a      | Fiscalité calculée (IR) + Prévoyance + Retraite                                           | F3, V2-08/10    | partiel             | Complète la fiscalité déclarative UX-03a.3 avec états moteurs réels ou « à venir » ; aucun hardcode fiscal                                                                                                          |
...
Ordre recommandé : UX-00a → UX-00a-bis → UX-00b → UX-01 → UX-02 → UX-03a.0 → UX-03a.1 → UX-03a.2 →
UX-03a.3 → UX-03a.4 → UX-03a.5 → UX-03a.6 → F1.1 → UX-03b → F2 → F3 → UX-05 → UX-06a → pilote IFI → UX-06b → F5 →
UX-04 → F6 → UX-07 → UX-08.
...
| ----------------------------------------------------- | ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ---------------- |
| IR disponible / TMI non synthétisée sur landing       | `src/features/audit/auditLandingViewModel.ts` expose `tmiLabel: 'IR disponible'`          | La landing renvoie vers `Fiscalité & budget` sans afficher de taux TMI global | UX-06a           |
