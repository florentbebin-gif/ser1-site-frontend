...
| Surface     | Statut                         | Role metier principal                                                                                                                                                  |
| ----------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/audit`    | actif, workflow prive P6       | Constituer un dossier patrimonial guide, persiste en session, sauvegardable localement, projete dans le dossier central F1 et servant encore d'entree a `strategy`     |
...
- Le `DossierPatrimonial` central F1 couvre le foyer, les membres, la situation familiale, le régime matrimonial, les donations synthétiques, les objectifs, contraintes, opérations prévues, `sourceRefs` minimales et le mini-jalon F1.1 `budgetSynthese` (ressources annuelles, charges annuelles, `sourceRefIds`). Il ne porte pas encore le graphe actif/passif complet F3, l'evidence enrichie F2, le modèle société/bilan F5 ni de nouveaux settings fiscaux.
- Depuis UX-03a, `/audit` expose les pages cockpit `Foyer & famille`, `Actifs / passifs`, `Fiscalité & budget` et `Objectifs`, dans un rail où `Sociétés / organigramme`, `Prévoyance`, `Succession` et `Synthèse & projection` restent positionnés comme jalons métier gated. `Actifs / passifs` reste un inventaire déclaratif à structurer F3 ; `Fiscalité & budget` estime l'IR/TMI via le moteur IR existant, qualifie l'IFI sans montant et affiche la synthèse budgétaire F1.1 sans pont `/sim/ir`. `Synthèse & projection` est verrouillée avant F6.
...
Le scénario annuel consiste à comparer la valeur officielle (BOFiP, Légifrance, BOSS/Ameli selon le
périmètre) avec la valeur administrée, puis à mettre à jour `verifiedAt` dans
`src/domain/settings-references/chain.json`. Une source attachée à un claim doit expliquer sa
pertinence ; une absence de source doit être représentée par `noRefReason`, jamais par une URL
générique.
