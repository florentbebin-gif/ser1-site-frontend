...
export interface UsePerPotentielReturn {
  ...
  updateDeclarant: (
    _scope: PerDeclarantScope,
    _decl: 1 | 2,
    _patch: Partial<DeclarantRevenus>,
  ) => void;
...
export function usePerPotentiel(
  ...
  const [state, setState] = useState<PerPotentielState>(
    () =>
      loadPerPotentielSession({ simplifiedMode }) ??
      makeDefaultPerPotentielState({ simplifiedMode }),
  );
  ...
  const setMode = useCallback(
    (mode: PerMode) => {
      ...
      const nextNeedsEstimate = mode === 'declaration-n1' ? false : state.needsCurrentYearEstimate;
  ...
  const setHistoricalBasis = useCallback(
  ...
  const setNeedsCurrentYearEstimate = useCallback(
  ...
  const updateAvisIr = useCallback(
    (patch: Partial<AvisIrPlafonds>, decl: 1 | 2 = 1) => {
  ...
  const updateSituation = useCallback(
    (patch: PerSituationPatch) => {
      persist({ ...state, ...patch });
    },
    [state, persist],
  );

  const updateProjectionSituation = useCallback(
    (patch: PerSituationPatch) => {
      persist({
        ...state,
        projectionFoyerEdited: true,
        projectionSituationFamiliale:
          patch.situationFamiliale ?? state.projectionSituationFamiliale,
        projectionIsole: patch.isole ?? state.projectionIsole,
        projectionMutualisationConjoints:
          patch.mutualisationConjoints ?? state.projectionMutualisationConjoints,
      });
    },
    [state, persist],
  );

  const updateDeclarant = useCallback(
    (scope: PerDeclarantScope, decl: 1 | 2, patch: Partial<DeclarantRevenus>) => {
      const key:
        | 'revenusN1Declarant1'
        | 'revenusN1Declarant2'
  ...
  const updateDeclarants = useCallback(
    (scope: PerDeclarantScope, patches: PerDeclarantPatch[]) => {
  ...
  const updateChildMode = useCallback(
    (childId: number, mode: PerChildDraft['mode']) => {
      persist({
        ...state,
        children: state.children.map((child) =>
          child.id === childId ? { ...child, mode } : child,
        ),
      });
    },
    [state, persist],
  ...
  const updateProjectionChildMode = useCallback(
    (childId: number, mode: PerChildDraft['mode']) => {
      persist({
        ...state,
        projectionFoyerEdited: true,
  ...
  const removeProjectionChild = useCallback(
    (childId: number) => {
      persist({
        ...state,
        projectionFoyerEdited: true,
  ...
  const buildInput = useCallback(
    (
      ...
      const historicalBasis =
        state.mode === 'declaration-n1' ? 'previous-avis-plus-n1' : state.historicalBasis;

      if (!historicalBasis) {
        return null;
      }

      const forceProjectionCouple =
        useProjection &&
