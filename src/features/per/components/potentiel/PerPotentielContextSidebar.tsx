...
const fmtPercent = (value: number): string => `${(value <= 1 ? value * 100 : value).toFixed(1)} %`;
...
function CompactMetricList({ items }: { items: MetricItem[] }): React.ReactElement {
  return (
...
export function PerPotentielContextSidebar({
  ...
  const madelinItems = result?.plafondMadelin
    ? [
        {
          label: 'Déclarant 1',
          value: fmtCurrency(result.plafondMadelin.declarant1.disponibleRestant),
        },
        ...(isCouple && result.plafondMadelin.declarant2
          ? [
              {
                label: 'Déclarant 2',
                value: fmtCurrency(result.plafondMadelin.declarant2.disponibleRestant),
              },
            ]
          : []),
      ]
  ...
  const projectionD1 = result
    ? [
        {
          label: 'Reliquat N-2',
          value: fmtCurrency(result.projectionAvisSuivant.declarant1.nonUtiliseN2),
        },
        {
          label: 'Reliquat N-1',
          value: fmtCurrency(result.projectionAvisSuivant.declarant1.nonUtiliseN1),
        },
        {
          label: 'Reliquat N',
  ...
  const projectionD2 = result?.projectionAvisSuivant.declarant2
    ? [
        {
          label: 'Reliquat N-2',
          value: fmtCurrency(result.projectionAvisSuivant.declarant2.nonUtiliseN2),
        },
        {
          label: 'Reliquat N-1',
          value: fmtCurrency(result.projectionAvisSuivant.declarant2.nonUtiliseN1),
        },
        {
          label: 'Reliquat N',
    ...
    <>
      <div className="premium-card per-potentiel-context-card sim-summary-card">
        ...
        <div className="per-potentiel-context-list">
          ...
          {parcoursPills.length > 0 && showPotentielAvis && (
            ...
            <div className="premium-card per-potentiel-context-card sim-summary-card sim-summary-card--secondary">
              ...
              <div className="per-sidebar-hero">
                ...
                <div className="per-potentiel-mini-kpis-row">
                  ...
                  <SecondaryMetric
                    label="6QR"
                    value={result.declaration2042.case6QR ? 'Oui' : 'Non'}
                  />
                </div>
              </div>
