    ...
    >
      <div className="per-transfert-modal-grid">
        <PerTransfertSelectField
          ...
          onChange={(value) => update('currentRentMode', value as PerTransfertFormState['currentRentMode'])}
          options={[
        ...
        {state.currentRentMode === 'manual_table' ? (
          <>
            ...
            />
            <PerTransfertRateField label="Taux technique" value={state.currentTechnicalRate} onChange={(value) => update('currentTechnicalRate', value)} />
            <PerTransfertRateField label="Frais conversion rente" value={state.currentConversionFeeRate} onChange={(value) => update('currentConversionFeeRate', value)} />
            <PerTransfertRateField label="Frais arrérages" value={state.currentArrearsFeeRate} onChange={(value) => update('currentArrearsFeeRate', value)} />
            <PerTransfertIntegerField label="Annuités garanties" value={state.currentGuaranteedYears} onChange={(value) => update('currentGuaranteedYears', value)} min={0} suffix="ans" />
            <PerTransfertRateField label="Taux de réversion" value={state.currentReversionRate} onChange={(value) => update('currentReversionRate', value)} />
          ...
          <p className="per-transfert-modal-note per-transfert-modal-grid__wide">
            SER1 conserve la rente brute annuelle du relevé comme rente estimée à l'âge de liquidation, puis applique uniquement la fiscalité et la revalorisation après liquidation.
          </p>
        )}
      </div>
    </SimModalShell>
