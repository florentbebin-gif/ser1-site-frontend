...
function donationEntreEpouxOptionLabel(
  option: SuccessionPatrimonialContext['donationEntreEpouxOption'],
): string {
...
export function buildSuccessionPatrimonialAnalysis(
  ...
  const donationsRapportables =
    donations.length > 0
      ? computedDonationsRapportables
      : asAmount(patrimonial.donationsRapportables);
  const donationsHorsPart =
    donations.length > 0 ? computedDonationsHorsPart : asAmount(patrimonial.donationsHorsPart);
  ...
  const hasTargetedPreciput =
    patrimonial.preciputMode === 'cible' &&
    patrimonial.preciputSelections.some(
      (selection) => selection.enabled && asAmount(selection.amount) > 0,
    );
  ...
  const depassementQuotiteMontant = Math.max(
    0,
    liberalitesImputeesMontant - quotiteDisponibleMontant,
  );

  const warnings: string[] = [];
  const qualifiedAssets = options?.assetEntries ?? [];
  const propresParNatureCount = qualifiedAssets.filter(
    (entry) => entry.legalNature === 'propre_par_nature',
  ).length;
  const cmaQualifiedMovables = qualifiedAssets.filter(
    (entry) =>
  ...
  if (depassementQuotiteMontant > 0 && nbEnfants > 0) {
    warnings.push(
      'Libéralités hors part + legs au-delà de la quotité disponible: risque de réduction civile.',
    );
  }
  ...
  if (patrimonial.donationEntreEpouxActive) {
    if (civil.situationMatrimoniale === 'marie') {
      warnings.push(
        `Donation entre époux active (${donationEntreEpouxOptionLabel(patrimonial.donationEntreEpouxOption)}): vérifier les options du conjoint survivant et les droits réservataires.`,
      );
      if (patrimonial.donationEntreEpouxOption === 'pleine_propriete_totale') {
        warnings.push(
          'Totalité en pleine propriété: option exceptionnelle, sous réserve de réduction et accord des descendants (art. 1094-1 C. civ.).',
        );
      }
    } else {
      warnings.push(
        'Donation entre époux active incohérente hors mariage (donnée conservée à titre indicatif).',
      );
    }
  }
  ...
  if (patrimonial.attributionBiensCommunsPct !== 50 && civil.situationMatrimoniale === 'marie') {
    warnings.push(
      `Attribution des biens communs au survivant: ${patrimonial.attributionBiensCommunsPct} % (vs 50 % en partage usuel).`,
    );
  }
  ...
  if (civil.regimeMatrimonial === 'communaute_universelle') {
    if (patrimonial.stipulationContraireCU && propresParNatureCount > 0) {
      ...
      warnings.push(
        "Communaute universelle: des biens sont qualifies 'propre par nature' mais restent integres a la masse commune simplifiee tant que la stipulation contraire n'est pas activee.",
      );
    }
  }
  ...
  if (preciputMontant > 0 || hasTargetedPreciput) {
    if (civil.situationMatrimoniale === 'marie') {
      ...
      warnings.push(
        'Préciput saisi hors mariage: non applicable en l’état (vérifier le contexte).',
      );
    }
  }
  ...
  if (patrimonial.attributionIntegrale) {
    if (civil.situationMatrimoniale === 'marie') {
      warnings.push(
        'Attribution intégrale activée: peut retarder la transmission aux descendants au second décès.',
      );
    } else {
      warnings.push(
        'Attribution intégrale activée hors mariage: non applicable en l’état (vérifier le contexte).',
      );
    }
  }
  ...
  if (donations.length > 0) {
    ...
    const donSommeArgentExonereCount = donations.filter(
      (entry) => entry.donSommeArgentExonere,
    ).length;

    if (usufruitCount > 0) {
      warnings.push(
        `${usufruitCount} donation(s) avec réserve d’usufruit: valorisation fiscale/civile à confirmer.`,
      );
    }
    if (donSommeArgentExonereCount > 0) {
      warnings.push(
        `${donSommeArgentExonereCount} don(s) de somme d’argent exonéré(s): vérifier les conditions d’âge et de délai (CGI art. 790 G).`,
      );
    }
    if (computedDonationsPartagees > 0) {
      warnings.push(
        `Donation(s)-partage présente(s) (valeur gelée CCV 1078, non rapportable au partage civil) — exclue(s) de la masse civile de référence. Imputation fine sur la réserve non modélisée.`,
      );
    }
    ...
    if (fiscalSnapshot) {
      ...
      if (recentDonationsCount > 0) {
        warnings.push(
          `${recentDonationsCount} donation(s) dans le rappel fiscal de ${fiscalSnapshot.donation.rappelFiscalAnnees} ans: reprise DMTG à contrôler.`,
        );
      }
    }
  }
  ...
  if (
    options?.testament?.active &&
    options.testament.dispositionType === 'legs_particulier' &&
    legsParticuliers > 0
  ) {
    warnings.push(
      `Legs particuliers integres au titre du testament du cote ${options.simulatedDeceased}.`,
    );
  }

  warnings.push(
    'Module patrimonial simplifié: rapport civil détaillé, réduction fine et liquidation notariale non modélisés.',
  );
