...
function getSoulteReceivedTotal(enfantId: string, soultes: SuccessionDonationPartageSoulte[]): number {
  return soultes
...
function buildLotNetValues(act: SuccessionDonationPartageAct): LotNetValue[] {
  return getAcceptedLots(act).map((lot) => {
    ...
    const valeurActuelle = lot.valeurActuelle == null
      ? undefined
      : asAmount(lot.valeurActuelle);
...
export function validateDonationPartageAct(
  ...
  const errors: string[] = [];
  const livingEnfantIds = new Set(enfants.filter((enfant) => !enfant.deceased).map((enfant) => enfant.id));
...
export function buildDonationPartageFiscalLines(
  ...
  return acts.flatMap((act) => buildLotNetValues(act)
...
export function summarizeDonationPartageActs(acts: SuccessionDonationPartageAct[]): string | null {
  ...
  const soulteTotal = acts.reduce(
    (sum, act) => sum + act.soultes.reduce((innerSum, soulte) => innerSum + asAmount(soulte.montant), 0),
    0,
  );
  ...
  const soulteLabel = soulteCount === 1
