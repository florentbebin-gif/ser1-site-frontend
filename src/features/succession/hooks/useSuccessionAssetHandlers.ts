...
export function useSuccessionAssetHandlers({
  ...
  const resolvePersonParty = useCallback(
    (pocket: SuccessionAssetPocket): SuccessionPersonParty => {
  ...
  const hasResidencePrincipale = useCallback(
    (entries: SuccessionAssetDetailEntry[], exceptId?: string) =>
      entries.some(
        (entry) =>
          entry.id !== exceptId &&
  ...
  const setSimplifiedBalanceField = useCallback(
    (type: 'actifs' | 'passifs', owner: SuccessionLegacyAssetOwner, value: number) => {
      setAssetEntries(
        buildAggregateAssetEntries(
          {
            actifs: {
              epoux1:
                owner === 'epoux1' && type === 'actifs'
                  ? Math.max(0, value)
                  : assetBreakdown.actifs.epoux1,
              epoux2:
                owner === 'epoux2' && type === 'actifs'
                  ? Math.max(0, value)
                  : assetBreakdown.actifs.epoux2,
              commun:
                owner === 'commun' && type === 'actifs'
                  ? Math.max(0, value)
                  : assetBreakdown.actifs.commun,
            },
            passifs: {
              epoux1:
                owner === 'epoux1' && type === 'passifs'
                  ? Math.max(0, value)
                  : assetBreakdown.passifs.epoux1,
              epoux2:
                owner === 'epoux2' && type === 'passifs'
                  ? Math.max(0, value)
                  : assetBreakdown.passifs.epoux2,
              commun:
                owner === 'commun' && type === 'passifs'
                  ? Math.max(0, value)
                  : assetBreakdown.passifs.commun,
            },
          },
          {
  ...
  const updateAssetEntry = useCallback(
    (id: string, field: keyof SuccessionAssetDetailEntry, value: string | number) => {
      if (field === 'subCategory') {
        ...
        if (value === 'GFA/GFV' || value === 'GFF/GF') {
          ...
          setGroupementFoncierEntries((prev) => [
            ...prev,
            buildGroupementFoncierFromAsset(sourceEntry, value),
          ]);
          return;
        }
      ...
      setAssetEntries((prev) =>
        prev.map((entry) => {
          ...
          if (field === 'subCategory') {
            const nextSubCategory =
              value === RESIDENCE_PRINCIPALE_SUBCATEGORY && hasResidencePrincipale(prev, id)
                ? RESIDENCE_SECONDAIRE_SUBCATEGORY
                : value;
            return {
              ...entry,
