...
import type { AssociateRevenuePhaseInputV6, CcaRepaymentStrategy } from '@/engine/tresorerie/types';
    ...
    <section className="ts-associate-card">
      ...
      <div className="ts-phase-source" role="radiogroup" aria-label="Remboursement annuel souhaité">
        {(
          ...
          ] as Array<[CcaRepaymentStrategy, string]>
        ).map(([value, label]) => (
      ...
      <p className="ts-note--info">
        Les retraits sont limités au solde CCA et à la trésorerie disponible après IS, solde minimum
        bancaire et BFR conservés.
      </p>
    </section>
  );
}
