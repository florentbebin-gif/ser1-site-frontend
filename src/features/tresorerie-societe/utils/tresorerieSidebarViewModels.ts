...
function sumSegments(
  ...
  const segments: TresoAssociateInsightSegment[] = [
    { key: 'remuneration', label: getTresoRevenueSourceLabel('remuneration'), value: remuneration / annualDivisor },
    { key: 'cca', label: getTresoRevenueSourceLabel('cca'), value: cca / annualDivisor },
    { key: 'dividendes', label: getTresoRevenueSourceLabel('dividendes'), value: dividendes / annualDivisor },
  ];
...
export function buildTresoAssociateInsightViewModel(
  ...
  const targetContext = rowsWithContext.find(item => item.annualNeed > 0)
    ?? rowsWithContext[retirementIndex]
    ?? rowsWithContext[0];
  const needContexts = rowsWithContext.filter(item => item.annualNeed > 0);
  const analysisContexts = needContexts.length > 0
    ? needContexts
    : targetContext
      ? [targetContext]
      : [];
  ...
  return {
    ...
    targetYear,
    targetAge: firstContext && profile.currentAge > 0 ? profile.currentAge + firstContext.row.year - 1 : null,
    periodLabel: firstContext && lastContext
