...
describe('computeTresoOrgchartLayout', () => {
  it('positionne un associé, la société et le lien de détention', () => {
    ...
    expect(layout.labels).toEqual(expect.arrayContaining([
      expect.objectContaining({ text: '100 %' }),
    ]));
  });
  ...
  it('répartit plusieurs associés sur une même rangée', () => {
    ...
    expect(layout.labels.map(label => label.text)).toEqual(expect.arrayContaining(['60 %', '40 %']));
  });
  ...
  it('connecte une filiale et une sous-filiale au bon parent', () => {
    ...
    expect(layout.labels.map(label => label.text)).toEqual(expect.arrayContaining(['80 %', '51 %']));
