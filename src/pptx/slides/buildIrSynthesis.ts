...
import { SLIDE_SIZE, TYPO, addHeader, addFooter, addTextFr } from '../designSystem/serenity';
import {
...
export function buildIrSynthesis(
  ...
  addHeader(
    ...
    'Principaux indicateurs fiscaux',
    theme,
    'content',
  );
  ...
  const kpiData: Array<{
  ...
  }> = [
    {
      ...
      subValue: data.isCouple ? `D1: ${euro(data.income1)} | D2: ${euro(data.income2)}` : undefined,
    },
    {
      icon: 'cheque',
  ...
  const marginValue =
    data.tmiMarginGlobal !== undefined && data.tmiMarginGlobal !== null
      ? data.tmiMarginGlobal
      : nextTmiInfo
        ? nextTmiInfo.margin * data.partsNb
        : null;
