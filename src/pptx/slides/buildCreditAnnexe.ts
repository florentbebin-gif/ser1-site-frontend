...
function buildCreditAnnexeProse(data: CreditAnnexeData): Array<{ text: string; bold?: boolean }[]> {
  ...
  if (isMultiLoan) {
    ...
    if (loan) {
      const creditTypeLabel =
        loan.creditType === 'amortissable' ? 'amortissable classique' : 'in fine';
      paragraphs.push([
  ...
  if (isMultiLoan) {
    loans.forEach((loan) => {
      ...
      const assurModeLabel =
        loan.assuranceMode === 'CI' ? 'sur le capital initial' : 'sur le capital restant dû';
  ...
  if (data.smoothingEnabled && isMultiLoan) {
    const smoothingLabel =
      data.smoothingMode === 'duree' ? 'durée constante' : 'mensualité constante';
    paragraphs.push([
...
export function buildCreditAnnexe(
  ...
  addHeader(
    slide,
    'Annexe : Détail de votre crédit',
    'Explication des modalités de remboursement',
    theme,
    'content',
  );
