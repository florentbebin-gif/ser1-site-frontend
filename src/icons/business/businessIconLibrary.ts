...
const svgModules = import.meta.glob('./svg/icon-*.svg', {
  eager: true,
  import: 'default',
  query: '?raw',
}) as Record<string, string>;

// Mapping des noms vers les SVG normalisés, dérivé des noms de fichiers.
const svgMap = Object.fromEntries(
  Object.entries(svgModules).map(([path, svg]) => [
    path.replace('./svg/icon-', '').replace('.svg', ''),
    svg,
  ]),
) as Record<BusinessIconName, string>;
