  ...
  return (
    <div className="app-error-page">
      <div className="app-error-card">
        <div className="app-error-icon">{isConfigError ? '🔧' : '⚠️'}</div>
        ...
        {errorMessage && <pre className="app-error-details">{errorMessage}</pre>}

        <button onClick={() => window.location.reload()} className="app-error-retry">
          Réessayer
        </button>
      </div>

      <div className="app-error-footer">SER1 &bull; Gestion Privée</div>
    </div>
  );
}
